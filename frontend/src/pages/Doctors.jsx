...
const Doctors = () => {
  ...
  return (
    <div>
      ...
      {/* Search Box - Our New Feature! */}
      <div className='mt-4 mb-2'>
        <input
          type='text'
          placeholder='Search by specialty e.g. Dermatologist...'
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          className='w-full sm:w-72 px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-primary'
        />
      </div>
      ...
      <div className='flex flex-col sm:flex-row items-start gap-5 mt-5'>
        ...
        <div className='w-full grid grid-cols-auto gap-4 gap-y-6'>
          {filterDoc.length > 0 ? filterDoc.map((item, index) => (
            ...
            <p className='text-gray-500 text-sm mt-4'>No doctors found for "{searchQuery}"</p>
          )}
        </div>
      </div>
    </div>
