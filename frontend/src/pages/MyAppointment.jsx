...
const MyAppointments = () => {
  ...
  const appointmentPayment = async (appointmentId) => {
    try {
      const { data } = await axios.post(backendUrl + '/api/user/payment-dummy', { appointmentId }, { headers: { token } })
      if (data.success) {
            ...
            <div className='flex flex-col gap-2 justify-end text-sm text-center'>
              ...
              {!item.cancelled && !item.payment && !item.isCompleted &&
                <button
                  ...
                  className='text-white sm:min-w-48 py-2 border rounded bg-primary hover:opacity-90 transition-all duration-300'>
                  Pay Online
                </button>
              }
